...
> 📥 **要接携程点评？** 看 `接入携程-浏览器助手.md`：装一个书签，在携程后台点一下即可把点评导入工具，不需要接口权限。
>
...
点页面左上角的图标即可上传酒店头像（点击更换，悬停点 × 移除）。头像会自动居中裁切为 200×200 并压缩后保存在本机，
并跟随「门店配置 → 导出 JSON」一起分享给同事。

**头像不受公开版锁定影响**：门店名称与电话在公开版是只读的，但**头像任何设备都能自行更换**（各改各的，只影响自己的浏览器）。
仓库根目录 `site-config.json` 的 `avatar` 字段（data URL）可作为**默认头像**；一旦某台设备自己改过，就以本机设置为准，不再被覆盖。
