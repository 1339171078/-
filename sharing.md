...
并忽略分享链接里携带的门店配置——同事不会误改你们的店名和电话。
...
- **公开版功能不受影响**：四维度诊断、三种语气、批量处理、导出 CSV/TXT、历史记录全部照常可用。
...
「🔗 分享链接」会把**当前门店配置 + 正在处理的这条点评**压缩后写进网址的 `#` 片段：

- 部署在内网/云端时：同事点开链接就能看到同一条点评的诊断与回复，适合"这条差评怎么回，帮我看看"的场景；
- 直接用 `file://` 打开时：链接只能在本机使用，此时工具会提示你改用"发送文件 + 配置 JSON"的方式。

链接里的内容只存在于 `#` 后面，**不会发送到任何服务器**（浏览器不会把 `#` 片段发给服务端）。
